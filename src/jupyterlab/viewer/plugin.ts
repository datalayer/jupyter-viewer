...
export const NOTEBOOK_VIEWER_WIDGET_FACTORY = 'Notebook Viewer';

const viewerPlugin: JupyterFrontEndPlugin<IViewerTracker> = {
  id: '@datalayer/jupyter-viewer:notebook',
  autoStart: true,
  ...
  activate: (
    ...
    const viwerRenderFactory = new ViewerFactory({
      name: NOTEBOOK_VIEWER_WIDGET_FACTORY,
      fileTypes: ['notebook'],
      modelName: 'notebook',
    });
      ...
      execute: async args => {
        ...
        if (notebookPanel) {
          ...
          commands.execute('docmanager:open', {
            path: context.path,
            factory: NOTEBOOK_VIEWER_WIDGET_FACTORY,
            options: {
              mode: 'split-right'
            }
          });
    ...
    console.log('JupyterLab plugin @datalayer/jupyter-viewer:notebook is activated.');
